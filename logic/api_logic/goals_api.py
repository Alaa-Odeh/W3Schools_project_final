...
class GoalsAPI:
    ...
    def get_created_goal_info(self,expected_skills):
        goal_id=self.get_goal_id()
        self.new_url = self.new_url + f'/{goal_id}'
        self.result=self.api_object.api_get_request(self.new_url).json()['skills']
        skills_dict = self.skills.get_skills_dict_by_id()
        levels_dict = self.skills.info_api["levels_dict"]
        skills_info = {}

        for skill_id, details in self.result.items():
        ...
        return ordered_skill_names, ordered_skill_levels